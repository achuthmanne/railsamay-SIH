...
const LiveEcosystem = () => {
  ...
  // Dynamic Visual Speed Calculations based on actual train speed (km/h)
  // Inverse relationship: Higher speed = Lower animation duration (faster visual movement)
  const baseSpeedRef = 100;
  const poleAnimDuration = `${(baseSpeedRef / speed) * 3}s`; // at 100km/h -> 3s, at 50km/h -> 6s
  const sceneryAnimDuration = `${(baseSpeedRef / speed) * 35}s`;
  const stationAnimDuration = `${(baseSpeedRef / speed) * 40}s`;
    ...
    <div className="w-full relative pb-20 bg-white">
      ...
      <div className={`relative w-full h-[600px] overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.1)] border-y-4 border-[#1E3A8A] transition-colors duration-[3000ms] ease-in-out ${bgColors[mode]}`}>
        ...
        <style>{`
          ...
          @keyframes slideRightFast { 0%, 80% { transform: translateX(-100vw); } 100% { transform: translateX(100vw); } }
          ...
          <div className="bg-white shadow-2xl w-[360px] rounded-sm font-inter flex flex-col relative z-10 overflow-hidden">
            ...
            <div className="p-4 relative z-10">
               ...
               <div className="mb-2.5 flex justify-between items-end border-b border-slate-200 pb-2">
                  <div>
                     <div className="text-slate-500 text-[11px] font-semibold mb-1 uppercase tracking-wider">Next Station</div>
                     <div className="text-[#1E3A8A] font-montserrat font-black text-xl leading-none">
                       Nagpur <span className="text-[#F97316] font-semibold text-sm ml-0.5">(NGP)</span>
                     </div>
                  </div>
                  <div className="text-right">
                     <div className="text-slate-400 text-[10px] font-semibold mb-1 uppercase tracking-wider">Sch. Arrival</div>
                     <div className="text-slate-600 font-mono font-bold text-lg leading-none tracking-tight">
                       14:30
                     </div>
                  </div>
               </div>
               ...
               <div className="grid grid-cols-2 gap-4 mb-2.5">
                  <div>
                     <div className="text-slate-500 text-[11px] font-semibold mb-1 uppercase tracking-wider">Current Speed</div>
                  ...
                  <div className="text-right">
                     <div className="text-slate-500 text-[11px] font-semibold mb-1 uppercase tracking-wider">Target ETA</div>
                     <div className="text-slate-800 font-bold text-3xl font-mono leading-none tracking-tight">
                       {etaTime}
                     </div>
                  </div>
        ...
        <div 
          ...
          style={{ animation: `slideRightSlow ${sceneryAnimDuration} linear infinite` }}
        >
          {[...Array(16)].map((_, i) => (
        ...
        <div 
          ...
          style={{ animation: `slideRightMed ${poleAnimDuration} linear infinite` }}
        >
          <div className="absolute top-[25px] w-full h-[1.5px] bg-gray-400 opacity-80"></div>
          {/* Vertical Droppers between the wires */}
          <div className="absolute top-[25px] w-full h-[25px] flex justify-around">
            {[...Array(60)].map((_, i) => (
              <div key={`dropper-${i}`} className="w-[1px] h-full bg-gray-400 opacity-50"></div>
            ))}
          </div>
        ...
        {/* 5. STATION BOARD */}
        <div className="absolute bottom-[80px] left-[-200px] h-[220px] mix-blend-multiply" style={{ animation: `slideRightFast ${stationAnimDuration} linear infinite 5s` }}>
        ...
        <div className="absolute bottom-[80px] left-[5%] lg:left-[15%] h-[200px]" style={{ animation: 'trainBounce 0.2s infinite alternate' }}>
           <div className="relative h-full flex items-end">
               ...
               <div className={`flex items-end h-full transition-opacity duration-[2000ms] ${mode !== 'night' ? 'opacity-100' : 'opacity-0'}`}>
                   <img src="/engine.png" className="h-[105%] mix-blend-multiply object-contain relative z-10 translate-x-[6px] translate-y-[26px]" alt="Engine" />
